...
post.hm <- plot_heatmap(physeq.simper.post, sample.label = "Treatment", sample.order = c("LowFat", "HighFat", "Saccharin", "Stevia"), taxa.label = "OTU_Classification", taxa.order = "Phylum", low = "#FFFFCC", high = "firebrick3", na.value = "#FFFFFC") +
  ...
  theme(panel.border=element_blank(),
        axis.text.x = element_text(angle=50, hjust=0.3, family="Arial", size=10),
...
time.hm <- plot_heatmap(physeq.simper.time, sample.label = "Treatment_Time", sample.order = c("LF_Pre", "LF_Post", "HF_Pre", "HF_Post", "SC_Pre", "SC_Post", "TV_Pre", "TV_Post"), taxa.label = "OTU_Classification", taxa.order = "Phylum", low = "#FFFFCC", high = "firebrick3", na.value = "#FFFFFC") +
  ...
  theme(panel.border=element_blank(),
        axis.text.x = element_text(angle=50, hjust=0.1, vjust=0.6, family="Arial", size=10, 
...
simper.post.treat <- read.csv("Simper_OTU_post.csv")
simper.post.treat.hm <- simper.post.treat.hm[,-6:-8]
rownames(simper.post.treat.hm) <- simper.post.treat.hm[,1]
#colnames(simper.post.treat.hm) <- simper.post.treat.hm[1,]
simper.post.treat.hm <- simper.post.treat.hm[,-1]
simper.post.treat.mat <- as.matrix(simper.post.treat[,-6:-8])
test <- data.frame(simper.post.treat[,8])
rownames(test) <- simper.post.treat[,1]

simper.post.treat.annot <- data.frame(simper.post.treat[,1])
simper.post.treat.annot[,2] <- simper.post.treat[,6]


test <- simper.post.treat %>%
  group_by(OTU, LowFat, HighFat, Saccharin, Stevia)


pheatmap(simper.post.treat.hm, cluster_rows=F, show_rownames=T, cluster_cols=F, treeheight_col=0, showcolnames=T, border_color="gray40", fontsize=12, width=3, height=3, color = colorRampPalette(brewer.pal(8, "YlOrRd"))(100), labels_row=simper.post.treat[,8], cellwidth=30, cellheight=30)


test <- read.csv("Simper_OTU_post_test.csv")
test$Relative_Abundance <- test$Relative_Abundance * 100
test$OTU <- ordered(test$OTU, levels=c("Otu00003", "Otu00004", "Otu00009", "Otu00009", "Otu00021", "Otu00023", "Otu00010", "Otu00012", "Otu00014", "Otu00018", "Otu00008", "Otu00002")



ggplot(data=test, aes(x=Treatment, y = OTU)) + geom_tile(aes(fill = Relative_Abundance), colour="gray40") +
  geom_text(aes(label=sprintf("%0.2f", round(Relative_Abundance, digits=2)))) +
  scale_fill_gradient(low = "#FFFFCC", high = "firebrick3") + ylab("OTU Classification") +
  scale_y_discrete(labels=test$Name)
# https://stackoverflow.com/questions/38369855/how-to-put-exact-number-of-decimal-places-on-label-ggplot-bar-chart/38370083
