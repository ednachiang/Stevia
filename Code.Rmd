...
# Combined Variables
adonis(fem.bray ~ Time + Treatment, data=fem.sampdf)
...
# Combined Variables
adonis(mal.bray ~ Time + Treatment, data=mal.sampdf)
...
pair.combos <- combn(x=levels(pair.sampdf$Treatment), m=2)
pair.bray.pvals <- data.frame(rep(999,10))
for( i in 1:ncol(pair.combos)){
  temp.physeq <- subset_samples(physeq.pair, Treatment %in% combos[,i])
