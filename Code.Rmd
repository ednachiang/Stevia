...
wilcox.test(RelAbund ~ Diet, data=fir.post.df)
...
### Bacteroidetes in Stevia, Pre vs. Post
bac.phy <- subset_taxa(phy99, Phylum == "Bacteroidetes")
bac.tv <- subset_samples(bac.phy, Treatment == "Stevia")
bac.tv.df <- data.frame(sample_data(bac.tv))
bac.tv.df$RelAbund <- t(data.frame(otu_table(bac.tv)))
# Wilcoxon (Paired)
wilcox.test(RelAbund ~ Time, data=bac.tv.df, paired=T, alternative = "greater")
  # p-value = 0.01172
  # Pre < Post
