...
all.nmds <- plot_ordination(physeq=physeq.pair, ordination=bray.nmds, color="Treatment", shape="Time") +
  ...
  annotate("text", x = 0.56, y = -1.05e-04, label = "stress = 0.00009", size=2, family="Arial") +
...
pre.nmds <- plot_ordination(physeq=physeq.pre, ordination=bray.nmds.pre, shape="Sex") +
  ...
  annotate("text", x = 0.31, y = -0.52, label = "stress = 0.181", size=2, family="Arial")
...
post.fem.hm <- plot_heatmap(physeq.simper.post.fem, sample.label = "Treatment", sample.order = c("LowFat", "HighFat", "Saccharin", "Stevia"), taxa.label = "OTU_Classification", taxa.order = "Phylum") +
  ...
  scale_fill_gradient(name=expression("Relative\nAbundance\n(%)"),
                      low = "#FFFFFF", high = "midnightblue", limits=c(0,24),
  ...
  theme(panel.border=element_blank(),
        axis.text.x = element_text(angle=50, hjust=1, vjust=1.2, family="Arial", size=8),
        axis.text.y = element_text(family="Arial", size=8),
        axis.ticks.x=element_blank(),
        axis.ticks.y=element_blank(),
        axis.title.x = element_text(size=8, face="bold", family="Arial"),
        axis.title.y = element_text(margin = margin(t=0, r=0, b=0, l=0), size=8, face="bold", family="Arial"),
...
post.mal.hm <- plot_heatmap(physeq.simper.post.mal, sample.label = "Treatment", sample.order = c("LowFat", "HighFat", "Saccharin", "Stevia"), taxa.label = "OTU_Classification", taxa.order = "Phylum") +
  ...
  #geom_text(aes(label=sprintf("%0.2f", round(Abundance, digits=2)))) +
  scale_fill_gradient(name=expression("Relative\nAbundance\n(%)"),
                      low = "#FFFFFF", high = "midnightblue", limits=c(0,13),
                      breaks=c(0, 3.25, 6.50, 9.75, 13), labels=c("0", "3.25", "6.50", "9.75", "13")) +
  scale_x_discrete(labels = c("Low Fat", "High Fat", "Saccharin", "Stevia")) +
...
time.fem.hm <- plot_heatmap(physeq.simper.time.fem, sample.label = "Treatment_Time", 
  ...
  theme(panel.border=element_blank(),
        axis.text.x = element_text(angle=50, hjust=1.1, vjust=1.18, family="Arial", size=8, 
...
time.mal.hm <- plot_heatmap(physeq.simper.time.mal, sample.label = "Treatment_Time",
  ...
  #geom_text(aes(label=sprintf("%0.2f", round(Abundance, digits=2)))) +
  scale_fill_gradient(name=expression("Relative\nAbundance\n(%)"),
                      low = "#FFFFFF", high = "midnightblue", limits=c(0,21),
                      breaks=c(0, 5.25, 10.5, 15.75, 21), labels=c("0", "5.25", "10.5", "15.75", "21")) +
  scale_x_discrete(labels = c("Low Fat Pre", "Low Fat Post", "High Fat Pre", "High Fat Post",
                              "Saccharin Pre", "Saccharin Post", "Stevia Pre", "Stevia Post")) +
...
### HeatMap Plot
```{r}
tiff("hm.tiff", width=180, height = 170, units="mm", res = 600)
grid.newpage()
pushViewport(viewport(layout=(grid.layout(4,2, widths=c(1,1), heights=c(0.05,1.00,0.05,1)))))
grid.text("A", x=unit(0.03, "npc"), vp=viewport(layout.pos.row=1, layout.pos.col=1),
...
grid.text("B", x=unit(0.03, "npc"), vp=viewport(layout.pos.row=1, layout.pos.col=2),
          gp=gpar(fontsize=16,fontface="bold",fontfamily="Arial"))
print(time.mal.hm, vp=viewport(layout.pos.row=2, layout.pos.col=2))
grid.text("C", x=unit(0.03, "npc"), vp=viewport(layout.pos.row=3, layout.pos.col=1),
          gp=gpar(fontsize=16,fontface="bold",fontfamily="Arial"))
print(post.fem.hm, vp=viewport(layout.pos.row=4, layout.pos.col=1))
grid.text("D", x=unit(0.03, "npc"), vp=viewport(layout.pos.row=3, layout.pos.col=2),
          gp=gpar(fontsize=16,fontface="bold",fontfamily="Arial"))
print(post.mal.hm, vp=viewport(layout.pos.row=4, layout.pos.col=2))
dev.off()
